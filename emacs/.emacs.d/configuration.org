...
#+AUTHOR: Dirk Schroetter
#+EMAIL: dschroetter@gmail.com
...
#+BEGIN_SRC emacs-lisp
  (setq user-full-name "Dirk Schroetter"
        user-mail-address "dschroetter@gmail.com"
        calendar-latitude 53.58
        calendar-longitude 9.97
...
I'm currently using the "leuven" theme. I've got a scenic wallpaper, so just a
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun hrs/apply-theme ()
    "Apply the `leuven' theme and make frames just slightly transparent."
    (interactive)
    (load-theme 'leuven t)
    (transparency 90))
#+END_SRC
...
Turn off VC as we will not use it when we have Magit
...
I'd like to always use Safari, so:
...
#+BEGIN_SRC emacs-lisp
  (use-package elfeed
    ...
    (progn
      ...
      (define-key elfeed-show-mode-map "&" 'ds/special-browse)))

  (defun ds/special-browse (&optional use-generic-p)
    "Visit the entry using the external browser"
    (interactive "P")
    (let ((link (elfeed-entry-link elfeed-show-entry)))
      (when link
        (eww-browse-with-external-browser link))))

  (defun ds/categories (entry)
    (message "Enc: %s" (elfeed-meta entry :Enclosure)))

  (defun ds/parse-categories (type entry db-entry)
    (let ((emptags (cdr (-flatten (xml-query-all '(tags) entry))))
          (empenc  (xml-query-all '(enclosure url) entry)))
         (progn (setf (elfeed-meta db-entry :ds/emptags) emptags))
                (setf (elfeed-meta db-entry :ds/enclosure) empenc)))

  ;; Mark all YouTube entries
  (add-hook 'elfeed-new-entry-hook #'ds/categories)
  (add-hook 'elfeed-new-entry-parse-hook #'ds/parse-categories)
...
#+END_SRC
* Org Mode
I want to have the Tufte document class available in Org Mode

Captions should always be placed below the element
...
Org Babel needs to understand LaTeX as well
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-latex-pdf-process (list "latexmk -shell-escape -bibtex -f -pdflua %f"))
